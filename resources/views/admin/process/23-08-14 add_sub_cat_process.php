...

{
	...
	$category_level  	 = trim($_REQUEST['category_level']);
...

	{
		...
		$errorstr .="Please Select Category Level\n";

		$case = 0;

	}
	elseif($category_level<2 && $category_level>5)
	{
			$errorstr .= "Please Select Valid Category Level(2-5)\n";
	...
	else
	{
		...
		if($parent_id=="")
		{
			$errorstr .="Please Select Level1 category\n";
			$case = 0;
		}
		else
		{
			$chk_level1_cat_qry ='select cat_id as chk_level1_id from tbl_categories where cat_id="'.$parent_id.'"
			and level=1';
			$chk_level1_cat_arr = $db->get_row($chk_level1_cat_qry, ARRAY_A);
			$chk_level1_id  = $chk_level1_cat_arr['chk_level1_id'];	
			if($chk_level1_id=="")
			{
				$errorstr .="Please Select Valid Level1 category\n";
				$case = 0;
			}
		}
		...
		if($category_level==3)
		{	
			if($level2_cat_id == "" && $chk_level1_id!="")
			{
				$errorstr .="Please Select Level2 Category\n";
				$case = 0;
			}
			elseif($level2_cat_id!= "" && $chk_level1_id!="")
			{
		...
		{	
			...
			elseif($level3_cat_id!="" && $chk_level2_cat != "")
			{
		...
		{	
			...
			elseif($level4_cat_id != "" && $chk_level3_cat!="")
			{
