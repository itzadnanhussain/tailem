...
if(isset($status) && !empty($status))
{
	...
	$db->query($sqlquery);
	header("Location:artist_album_list.php?artist_id=$artist_id&msg=$update_ok_msg&case=1");
	exit;
}
?>
									...
									?>
									  ...
									  <?php
											...
											if($_GET['artist_id']!=""){
												...
												$targetpage = "artist_album_list.php?artist_id=".$_GET['artist_id']; 
											}else{
												
												$targetpage = "artist_album_list.php"; 
											}	
								 ...
								 if(isset($artist_list_arr))
								  ...
								  <tr bgcolor="<?php echo $bgcolor; ?>" onMouseOver="changebackcolor_hover('row<?php echo $id;?>')" onMouseOut="changebackcolor_blur('row<?php echo $id;?>')" id="row<?php echo $id;?>">
									...
									<?php
										...
										{
											?>
											 <img src="../site_upload/album_images/<?php echo $album_picture;?>"  border="0" style="width:70px;"/>
											<?php
										}
										else
										{
									...
									if($status==1)
									{
									  ...
									  <?php
										if($status==0)
										{
											echo '<a href="artist_album_list.php?status='.base64_encode(1).'&status_id='.base64_encode($id).'&artist_id='.$_REQUEST['artist_id'].'"><img src="images/disable.gif" border="0" class="Action" title="Activate"></a>'; 
										}
										if($status==1)
										{
											echo '<a href="artist_album_list.php?status='.base64_encode(0).'&status_id='.base64_encode($id).'&artist_id='.$_REQUEST['artist_id'].'"><img src="images/enable.gif" border="0" class="Action" title="Blocked"></a>'; 
										}
									  ?></td>
...
&nbsp;&nbsp; <input type="checkbox" class="check-all" name="ids[]" id="ids[]" value="<?php echo base64_encode($id);?>" style="margin-top:-5px;" />
									...
									{
									?>
									<a href="addedit_artist_album.php?edit_id=<?php echo base64_encode($id);?>&artist_id=<?php echo $_REQUEST['artist_id'];?>"><img src="images/edit.gif" border="0" title="Edit" class="Action"></a>
									 <?php
									}
									?>
                                  ...
                                  <a href="artist_album_songs_list.php?artist_id=<?php echo $_REQUEST['artist_id'];?>&album_id=<?php echo base64_encode($id);?>"><img src="images/music.png" width="20" border="0" title="Album Songs" class="Action" ></a>
								  ...
								  <?php
									...
									?>
									  ...
									  ?>
									  <tr>
                                        <td colspan="7" align="center" valign="middle"><?php include("common/paging-sub.php"); ?></td>
                                      </tr>
									  </form>
