...
if($_REQUEST['del_id']!="")
{
	...
	$select_img ="select profile_image from tbl_users where user_id='".base64_decode($_REQUEST['del_id'])."'";
	...
	if($old_image!="")
	{
		...
		mysqli_query($db->dbh, "update tbl_users set profile_image = '' where user_id='".base64_decode($_REQUEST['del_id'])."'");
		
	}
	header('Location: '.SERVER_ADMIN_PATH.'addedit_user.php?edit_id='.$_REQUEST['del_id']);
}

if($edit_id!=""){
	
	$edit_id = base64_decode($edit_id);
	$row  =  $db->get_row("select user_id,user_name,fname,lname,gender,user_email, about_me, country_id, region, 
	profile_image from tbl_users where user_id='".$edit_id."'",ARRAY_A);
	$user_id 	     = $row['user_id'];
	...
	$gender          = $row['gender'];
	$user_email 	 = stripslashes(html_entity_decode($row['user_email']));
...
<script type="text/javascript">
	function delete_image_user(id) // for changing multiple status or multiple delete 
	{
		...
		if(conBox)
		{
			window.location.href = "<?php echo SERVER_ADMIN_PATH;?>addedit_user.php?del_id="+id;	
		}			  
	}
</script>
											...
											<table id="Table1" border="0" cellpadding="0" cellspacing="0" width="100%">
											  <tbody>
												<tr>
												  ...
												  &raquo; <a href="<?php echo SERVER_ADMIN_PATH;?>users_list.php">Users Listing</a>
              ...
              <table class="Panel">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td width="88%">
                    <input type="password" name="simple_password" id="simple_password" class="fields" 
                    autocomplete="off"/>
                    <span class="Required">*</span>
                    </td>
                  </tr>
