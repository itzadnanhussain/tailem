...
$_SESSION['backtopage'] = $_SERVER['HTTP_REFERER'];
if($_REQUEST['del_id']!="")
{
	...
	if($old_image!="")
	{
		...
		mysqli_query($db->dbh, "update tbl_songs set picture = '' where id='".base64_decode($_REQUEST['del_id'])."'");
		
	}
	header('Location: '.SERVER_ADMIN_PATH.'addedit_song.php?edit_id='.$_REQUEST['del_id']);
}
...
if($edit_id!="")
{
	...
	$row  =  $db->get_row("select * from tbl_songs where id='".$edit_id."'",ARRAY_A);
	...
	$row_result  =  $db->get_results("select `tbl_songs_artist`.artist_id,`tbl_artists`.artist_name from tbl_songs_artist inner join tbl_artists on `tbl_songs_artist`.`artist_id`= tbl_artists.id where song_id='".$edit_id."'",ARRAY_A);
	...
	//	$song_image_fm = $XmlObj->track->album->image[2];
		...
		$description1=mysqli_escape_string($db->dbh, stripslashes($description1));
...
<script type="text/javascript">
	function delete_song_image(id) // for changing multiple status or multiple delete 
	{
		...
		if(conBox)
		{
			window.location.href = "<?php echo SERVER_ADMIN_PATH;?>addedit_song.php?del_id="+id;	
		}			  
	}
</script>
<?php
											  ...
											  <tbody>
												<tr>
												  ...
												  <a href="<?php echo SERVER_ADMIN_PATH;?>index.php">Home</a>
													&raquo;<a href="<?php echo SERVER_ADMIN_PATH;?>song_list.php">Song Listing</a>
													<?php
													if($edit_id != '')
													{
													?>
				...
				<table class="Panel">
					<tbody>
                        ...
                        <select name="years">
                            ...
                            <?php
                                ...
                                {
                                $sel='';
                                if($k==$years){ $sel="selected='selected'";}
                                ?>
					  ...
					  <tr>
                        ...
                        <tr>
													...
													<td width="88%">
													<input type="text" name="itunes_url" id="itunes_url" value="<?php echo $itunes_url;?>" class="fields"> 	</td>
															  </tr>
                                          <tr>
													<td width="12%" nowrap="nowrap" class="SmallFieldLabel2">Buy Amazon URL:</td>
													<td width="88%">
													<input type="text" name="amazon_url" id="amazon_url" value="<?php echo $amazon_url;?>" class="fields"> 	</td>
															  </tr>
                                     <!--     <tr>
													<td width="12%" nowrap="nowrap" class="SmallFieldLabel2">Buy Google Play URL:</td>
													<td width="88%">
													<input type="text" name="google_url" id="google_url" value="<?php //echo $google_url;?>" class="fields"> 	</td>
											  </tr>-->
                      <tr>
				 ...
				 <tr>
					...
					<tr>
						 ...
						 <td width="88%">
							<input type="number" class="fields"  id="song_ranking" name="song_ranking" 
							value="<?php echo $ranking_order; ?>" >
                         </td>
					  </tr>
